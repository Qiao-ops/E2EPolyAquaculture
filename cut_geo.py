...
def cut_image_with_geoinfo(image_path, label_path, block_size=2048, stride=1024, save_img_path=None, save_label_path=None, sample_threshold=0.2):
    with rasterio.open(image_path) as src_img, rasterio.open(label_path) as src_label:
        ...
        stretched_image_path = "./data/lyg/geo/stretched_image.tif"
...
#                         dst_img.write(img_block)
                        dst_img.write(np.transpose(img_block, (2, 0, 1)))


                if save_label_path:
                    output_label_path = os.path.join(save_label_path, f"label_{num}.tif")
...
# 输入图像和标签路径
image_path = "./data/lyg/img/All.tif"
label_path = "./data/lyg/gt/All_label.tif"

# 保存拉伸后图像的路径
stretched_image_dir = "./data/lyg/geo/stretched/"
os.makedirs(stretched_image_dir, exist_ok=True)

# 保存裁切后图像和标签的路径
save_img_path = "./data/lyg/geo/cut_2048_u8_tt/img/"
save_label_path = "./data/lyg/geo/cut_2048_u8_tt/gt/"
...
cut_image_with_geoinfo(
    ...
    sample_threshold=0.2  # 样本比例阈值设置为 20%
)
