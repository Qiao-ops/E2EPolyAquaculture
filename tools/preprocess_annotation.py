...
def preprocess_annotation(json_path, save_path, num_corners, is_training=True):
    ...
    for i, anno in enumerate(tqdm(annotations, desc="Processing annotations")):
        ...
        if len(gt_pts) < 3:
            continue  # Skip this annotation if it has less than 3 vertices
        ...
        annotations[i]["bbox"] = get_gt_bboxes(gt_pts)
...
# 确保其他函数（如remove_doubles, get_gt_bboxes等）已正确定义
# def preprocess_annotation(json_path, save_path, num_corners, is_training=True):
#     """
#     Preprocess COCO-format annotations by cleaning noisy/invalid data and, for training data,
#     padding polygon and adding ground truth corner classes.
...
if __name__ == '__main__':
    # 定义参数
    json_path = '../data/yangzhiqu/lyg_3/cut_300/val/annotation.json'  # 原始标注文件的路径
    save_path = '../data/yangzhiqu/lyg_3/cut_300/val/annotation_processed4.json'  # 预处理后标注文件的保存路径
    num_corners = 4  # 您想要采样的顶点数
    is_training = True # 表示是否为训练数据集
